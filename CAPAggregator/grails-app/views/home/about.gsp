...
<head>
    ...
    <title>About CAP Aggregator : Middleware for aggregating emergency alerts from many heterogeneous sources</title>
...
<body>
  ...
  <h1>About CAPAggregator</h1>

  <p>
    CAPAggregator is an open source project resulting from volunteer effort
...
<a href="https://www.preparecenter.org/th/member-directory/eliotchristian">Eliots page at preparecentre </a> 
  ...
  <p>
    ...
    turn polled RSS and ATOM into push streams of events. This separation led to the development of CAPAggregator sister project : <a href="https://github.com/SemwebServices/PubSubHubBubFacade">feedFacade</a>. Initially
  ...
  <p>
    Eliots original CAP Alert Hub itself is extremely resilient and low resource, using lambda events to update static files of alerts. CAPAggregator takes a slightly different approach and is an attempt at
    leveraging the real-time features of an engine like elasticsearch to see how we might innovate in this space. CAPAggregator consumes CAP events from feedFacade and indexes all published alerts in real time where
    they can be searched as a single homogenous whole. Alerts are tagged with the Alert Hub subscriptions they match, and that tagging forms the basis of one of the views in the CAPAggregator interface.
    That view is, however, just a logical separation of the current index of live global alerts.
  </p>

  <p>
    CAPAggregator then is an attempt to create a single unified index of current live CAP events at the lowest possible granularity (Single Events). It leverages RabbitMQ to subscribe a queue to a topic, and elasticsearch
    as a real time indexing engine. In essence, CAPAggregator is a middleware software solution that sits between CAP Event Producers and CAP Event Consumers. The system bridges the gap created by the fact that our
...
<ul>
  <li><a href="https://demo.semweb.co/CAPCollator">CAPAggregator demo</a></li>
...
<p>
  <ul>
    <li><a href="https://github.com/SemwebServices/CAPCollator">CAPAggregator</a></li>
